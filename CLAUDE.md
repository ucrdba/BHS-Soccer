...
npm test           # vitest — 3,780 tests, config in vitest.config.mts
npm run preview    # serve dist/
```

Verification is three gates, and each covers a different slice:

- `npm test` — Vitest (3,780 tests across 206 files), including Vue component and database tests.
...
**A W/D/L sheet fills in one press.** A small-sided session is two sides of the same squad, so twenty-five dropdowns is the paper-beats-screen problem in its purest form: the grid offers Won/Drew/Lost above it, and the coach sets the squad then flips the side that won. `fillBlankOutcomes` fills **blanks only** — a result already chosen is the coach's, so a press late in entry cannot wipe the rows already done — and skips anyone not marked here, since an outcome implies attendance and filling an absent row would credit them for a game they did not play. The control appears only for `win_loss`: there is no sensible squad-wide Cooper's distance. **Reset** beside it empties the Result column and nothing else — attendance is a separate fact, and returning rows to the default would undo absences already marked — and it confirms first whenever there is a result to lose, because it sits one tap from Lost. **The Won / Drew / Lost boxes** take recording numbers as the paper sheet lists them (`17, 21, 11, 19`) and are the opposite of the fill: `applyOutcomeLists` **replaces** a listed player's result and marks them here, since naming the number is the choice, and leaves everyone unlisted alone. It applies on a press, never as typed — `17, 2` on the way to `17, 21` would set player 2 — and refuses the whole list, changing nothing, for a number in two lists or one nobody on the sheet has.
...
- **`promote_confirmed_profile` is the only place an invitation is redeemed at sign-up, and it runs when the email is confirmed.** Redeeming an invitation before that would let anyone who knows a player's address sign up as them and be put on the team. The redeeming itself is `redeem_invitations(user, take_school)` — at confirmation the first redeemed invitation's organization replaces the one taken from the team picked at sign-up; at sign-in an existing organization is kept — which skips an invitation to a deleted team, a roster entry that has left the team or been claimed, and an account already linked to a different roster entry. `promote_confirmed_profile` only touches a profile still at `pending_verification`, so a late confirmation cannot knock a settled account back into the queue. It matches invitations against `lower(auth.users.email)` — the address the confirmation just proved — never `profiles.email`, which a visitor could otherwise edit toward someone else's invited address; the privileged-column guard already refuses a visitor changing their own `profiles.email`, which is what keeps that address trustworthy. **"Confirm email" must stay on** — with it off, every account arrives confirmed and invitations are redeemed at sign-up.
- **Confirming clears the password typed at sign-up.** An unconfirmed account keeps its *first* sign-up's password, so someone who signs up first with another person's address could sign in as them after the real owner opens the confirmation link — verified on a local Supabase stack. `handle_user_confirmed` therefore sets `auth.users.encrypted_password = ''` when a confirmation email was sent (`confirmation_sent_at` is set), and the app asks for a password straight after a confirmation link (`AuthManager.beginPasswordSetup()`). It is not done at insert or when no link was sent: GoTrue's admin create confirms in a second statement, and those accounts' passwords — the demo's, for one — are meant to work.
- **An account that already existed when it was invited is connected at sign-in.** Confirmation only redeems for a brand-new account, so `AuthManager` calls `redeem_my_invitations()` after signing in and when a session restores an active profile, and reloads the profile if anything was redeemed. It refuses an account that is not active or whose address is not confirmed; a failure is ignored rather than shown, because the person did not ask for it.
- **Sign-in redemption refuses an account whose email address has changed.** `note_email_change`, a trigger on `auth.users`, stamps `profiles.email_changed_at` whenever the address changes, and `redeem_my_invitations()` refuses any account carrying it — otherwise an account holder could change their address to an invited one and take that place. Verified on the local Supabase stack, where one click by the account holder was enough to change the address even with Secure email change on (`double_confirm_changes = true`); the invitee did nothing, so the protection depends on neither. There is no admin screen to connect such an account: after checking the person owns the address, an admin clears `email_changed_at` in SQL (runbook, Troubleshooting). Confirmation has a second, order-independent check: `promote_confirmed_profile` redeems nothing, and files the account for approval, when `auth.users.email` no longer matches the sign-up address in `profiles.email`.
- **Every privileged account action is a `security definer` function that checks its caller** — the writes `create_invitation`, `revoke_invitation`, `approve_player_request`, `approve_coach_request`, `reject_request` and `redeem_my_invitations`, and the reads `pending_requests` and `team_linked_players`. `redeem_invitations` and `promote_confirmed_profile` trust the id they are given, so EXECUTE on both is revoked from every visitor role. A coach handles players on their own team; only an admin handles coaches — and an admin may approve any waiting request as a player or a coach, including one whose `requested_role` is `guest`, `admin` or null, which the queue marks "asked for no team role". `pending_requests()` returns only what the caller may act on; do not filter the queue in the browser instead. The PL/pgSQL functions that write — `create_invitation`, `revoke_invitation`, `approve_player_request`, `approve_coach_request`, `reject_request` — read `coalesce(public.current_profile_role(), 'guest')` or `coalesce(public.is_team_coach(x), false)`, never a bare comparison: a signed-in caller with no `profiles` row makes both helpers return NULL, and in PL/pgSQL a NULL condition inside an `if` skips the `raise` instead of tripping it, silently granting the privileged path rather than refusing it. The two read functions, `pending_requests()` and `team_linked_players()`, use the bare form and are safe only because their check sits in a `WHERE` clause, where a NULL excludes the row instead of skipping a raise — that difference does not carry over to a write function, so a new one has to fail closed the coalesce way, not copy the read functions' style.
- **`guard_profile_privileged_columns` is `security invoker` and lets a change through when `current_user` is not `anon` or `authenticated`.** Inside a definer function `auth.uid()` is still the coach's — it comes from the JWT — so the old `auth.uid()`-only test refused every coach approval. It must not name the `auth` schema itself: as the visitor, that lookup needs schema USAGE, so it asks `public.current_profile_role()` (a definer) instead. The guarded columns are `role`, `status`, `school_id`, `email`, `email_changed_at`, `player_id`, `requested_role` and `requested_team_id`: a visitor who could set their own `player_id` could attach themselves to any roster entry, one who could set `email` could walk a pending account onto an invited address, one who could clear `email_changed_at` could put a changed address back in reach of sign-in redemption, and one who could set `requested_role` could turn a waiting player request into a coach's.
- **Linking a roster entry locks the player row first.** `approve_player_request` and `redeem_invitations` both run `select … from public.players where id = … for update` before checking whether that player already has an account, so a coach approving a request and an invited player confirming — onto the same roster entry, at the same moment — serialize instead of racing each other onto it. There is deliberately no unique index on `profiles.player_id` yet: one could abort the migration against production data nobody has audited. That is a follow-up, not an oversight.
- **Invitation emails live in `invitations`, never on `players`**, which is publicly readable. The app sends no invitation email; `InviteControl` shows a `#signup=<address>` link (`domain/signup-link.ts`) for the coach to send. The address rides in the hash so it never reaches a server log, and `AppHeader` removes it from the URL once read — many of these addresses are minors'. Nothing is invited from the demo deployment: `InviteControl` is not rendered there.
- **A password-reset link — or a confirmation link — puts the app into a password prompt, and either backing out or signing out clears it.** `passwordPurpose()` says which (`reset` or `setup`), and the prompt words itself to match. "Not now" (or "Later") on the set-password screen calls `cancelPasswordRecovery()`, and `AuthManager.logout()` clears the same flag — neither leaves the app stuck asking for a new password from someone who never asked to set one.
...
**An edit of an existing drill is an update by id, never the upsert.** Every save used to go through `upsert(..., { onConflict: 'school_id,name' })` with the drill's id in the payload, which cannot rename: the new name matches no row, so the conflict does not fire, Postgres inserts, and the insert's id collides with the drill's own primary key (`23505 drills_bank_pkey`) — `on conflict` handles only the constraint it names. Renaming "1v1 Gauntlet (Continuous)" failed exactly that way. The upsert on `(school_id, name)` remains for a **new** drill only, where there is no id yet and the import relies on a repeated name updating its row. The update leaves `school_id` and `id` out of the body, since naming the row by id means either could only move it. `src/data/testdb/drill-rename.test.ts` proves both statements against Postgres.
