 ...
 *
 * Converted from js/diagrammer.js.
 */

import type {
  DiagramElement, DiagramDrawing, DiagramPoint, DiagramKeyframe, DiagramData,
  DiagramTool, PitchType
} from './types';
import type { BHSSoccerApp } from './app.core';
...
export class SoccerTacticalBoard {
  app: BHSSoccerApp;
  ...
  isDrawing: boolean;
  currentPath: DiagramDrawing | null;
  draggedElement: DiagramElement | null;
  draggedDrawing: DiagramDrawing | null;
  lastDragPos: DiagramPoint | null;
  selectedElement: DiagramElement | null;
  selectedDrawing: DiagramDrawing | null;
  dragOffset: DiagramPoint;
  hasAttached: boolean;
  hasKeydownAttached: boolean;
  boundCanvas?: HTMLCanvasElement | null;

  // Tactical Keyframe Animation Properties
  keyframes: DiagramKeyframe[];
  currentFrameIndex: number;
  isPlaying: boolean;
  animReqId: number | null;

  constructor(appInstance: BHSSoccerApp) {
  ...
  propagateNewElementsForward(): void {
  ...
  interpolateFrames(elementsA: DiagramElement[], elementsB: DiagramElement[], t: number): DiagramElement[] {
    const lerp = (a: number, b: number, progress: number) => a + (b - a) * progress;

    const result: DiagramElement[] = [];
    const usedB = new Set<DiagramElement>();
  ...
  loadDiagramData(data: DiagramData | string | null | undefined): void {
    if (!data) {
      this.elements = [];
      this.drawings = [];
      this.keyframes = [{
    ...
    if (parsed && parsed.keyframes && Array.isArray(parsed.keyframes) && parsed.keyframes.length > 0) {
      this.keyframes = JSON.parse(JSON.stringify(parsed.keyframes));
      this.currentFrameIndex = Math.min(parsed.currentFrameIndex || 0, this.keyframes.length - 1);
      const frame = this.keyframes[this.currentFrameIndex];
      this.elements = frame && frame.elements ? JSON.parse(JSON.stringify(frame.elements)) : (parsed.elements ? JSON.parse(JSON.stringify(parsed.elements)) : []);
      this.drawings = frame && frame.drawings ? JSON.parse(JSON.stringify(frame.drawings)) : (parsed.drawings ? JSON.parse(JSON.stringify(parsed.drawings)) : []);
    } else if (parsed) {
      this.elements = parsed.elements ? JSON.parse(JSON.stringify(parsed.elements)) : [];
      this.drawings = parsed.drawings ? JSON.parse(JSON.stringify(parsed.drawings)) : [];
      this.keyframes = [{
        time: 0,
        label: 'Time 0 (Start Position)',
        elements: JSON.parse(JSON.stringify(this.elements)),
    ...
    this.pitchType = (parsed && parsed.pitchType) || 'full';
    this.render();
    this.updateTimelineUI();
    this.updateToolbarUI();
  }

  getPos(e: PointerLikeEvent): DiagramPoint {
    const canvas = this.canvas!;
    const rect = canvas.getBoundingClientRect();
    const touchEvent = e as TouchEvent;
    const mouseEvent = e as MouseEvent;
    const hasTouches = touchEvent.touches && touchEvent.touches.length > 0;
    const clientX = hasTouches ? touchEvent.touches[0].clientX : mouseEvent.clientX;
    const clientY = hasTouches ? touchEvent.touches[0].clientY : mouseEvent.clientY;
    return {
  ...
  distToSegment(p: DiagramPoint, v: DiagramPoint, w: DiagramPoint): number {
  ...
  isPointNearDrawing(pos: DiagramPoint, drawing: DiagramDrawing, maxDist: number = 18): boolean {
  ...
  attachEvents(): void {
    ...
    const start = (e: PointerLikeEvent) => {
      if (!this.canvas) return;
      const pos = this.getPos(e);

      if ((['attacker', 'defender', 'gk', 'ball', 'cone', 'goal'] as DiagramTool[]).includes(this.activeTool)) {
        this.saveState();
        let num = '';
        ...
        const newEl: DiagramElement = {
          ...
          type: this.activeTool as DiagramElement['type'],
          x: pos.x,
          y: pos.y,
      ...
      } else if ((['line_solid', 'line_arrow', 'line_dribble', 'line_dashed', 'line_shot'] as DiagramTool[]).includes(this.activeTool)) {
        this.saveState();
        this.isDrawing = true;
        this.currentPath = {
          tool: this.activeTool as DiagramDrawing['tool'],
    ...
    this.canvas.addEventListener('mousedown', start as EventListener);
    this.canvas.addEventListener('mousemove', move as EventListener);
    window.addEventListener('mouseup', end);

    this.canvas.addEventListener('touchstart', (e: TouchEvent) => { e.preventDefault(); start(e); });
    this.canvas.addEventListener('touchmove', (e: TouchEvent) => { e.preventDefault(); move(e); });
  ...
  drawPitch(w: number, h: number): void {
    const ctx = this.ctx!;
  ...
  drawPath(d: DiagramDrawing): void {
  ...
  drawElement(el: DiagramElement): void {
    const ctx = this.ctx!;
    ctx.save();
