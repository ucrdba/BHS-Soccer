 ...
 *
 * Moved verbatim out of public/js/views/help.view.js during Phase 2 of the Vue
 * migration. Deliberately still HTML: it is 700 lines of authored prose, and
 * rewriting it into structured data would risk losing content for no gain the
 * reader would notice. It is repo-authored rather than user input, which is
 * what makes rendering it with v-html the right call rather than a shortcut.
 *
 * Both apps can read this. src/data/help-content.test.ts and
 * help-coverage.test.ts assert its SUBSTANCE -- that a screen which exists is
 * described somewhere -- rather than its wording, so it can be rewritten
 * freely as long as nothing goes uncovered.
 */

export interface HelpRole {
  /** 'coach', 'admin' or 'all'. Widened to string because several sections
   *  define their roles inline, where TypeScript widens the literal anyway. */
  kind: string;
  label: string;
}
...
export interface HelpSection {
  id: string;
  /** The group it appears under in the index. */
  part: string;
  title: string;
  roles: HelpRole[];
  /** Authored HTML. */
...
export function helpPath(...steps: string[]): string {
  return `<p class="help-path">${steps.map(t => `<b>${t}</b>`).join(' <span>&rsaquo;</span> ')}</p>`;
}

export function helpNote(label: string, html: string): string {
  return `<div class="help-note"><span class="help-note-label">${label}</span>${html}</div>`;
}

export function helpWarn(label: string, html: string): string {
  return `<div class="help-warn"><span class="help-warn-label">${label}</span>${html}</div>`;
}

export function helpTable(headers: string[], rows: string[][]): string {
  return `
...
// Short names, because the content below calls them several hundred times.
const P = (...s: string[]) => helpPath(...s);
const N = (l: string, h: string) => helpNote(l, h);
const W = (l: string, h: string) => helpWarn(l, h);
const T = (h: string[], r: string[][]) => helpTable(h, r);

const COACH: HelpRole = { kind: 'coach', label: 'Coach' };
const ADMIN: HelpRole = { kind: 'admin', label: 'Admin' };
const ALL: HelpRole = { kind: 'all', label: 'Everyone' };
