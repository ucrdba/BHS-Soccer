...
- **The two-migration order is load-bearing.** `0005` creates and copies; the application change deploys; verification runs; **only then** `0006` drops the old columns. Applying `0005` without deploying the code means the app keeps writing to `players.number` and `players.season_stats`, and `0006` deletes those writes silently.
